 ...
 * @props: env（指定xform组件当前运行的环境，dev表示开发环境、prod表示生产环境；默认值为生产环境） 
 * locale（当前使用的语言） 
 * popupContainer（xform组件内部使用的popup渲染的根节点，默认为xform组件的Root Element） 
 * widgets(为react-jsonschema-form注册的自定义widget) 
 * fields(为react-jsonschema-form注册的自定义field) 
 * fieldTemplate（为react-jsonschema-form注册的自定义普通类型字段模板）
 * arrayFieldTemplate(为react-jsonschema-form注册的自定义array类型字段模板) 
 * objectFieldTemplate(为react-jsonschema-form注册的自定义object类型字段模板) 
 * formContext（为react-jsonschema-form注册的formContext，通过这个上下文属性，在自定义子组件中可以获取到这些数据） 
 * defaultSubmitButtonComponent(渲染组件自定义的默认提交按钮，必填)     
...
class XFormCore extends Component {
    ...
    _validate(formData, name = 'all', callback, failCallback) {
            ...
            for (fieldName in uiSchema) {
                if (uiSchema.hasOwnProperty(fieldName)) {
                    ...
                    if (jsonSchemaContent.type === 'object' && jsonSchemaContent.hasOwnProperty('properties')) {
                        const childJsonSchemaProperties = jsonSchemaContent.properties;
                        let childJsonSchemaContent, childFieldValue, childFieldValidate = [], childClassNames = '', childWidgetType = '';
                        let childValidateItemPromises = [], childValidatePromises = [];
                        for (let child in childJsonSchemaProperties) {
                            if (uiSchema[fieldName].hasOwnProperty(child)) {
                                ...
                                let rPromise = _groupPromise(this, child, childValidateItemPromises, fieldName, groupHasItem);
                    ...
                    //如果是分组，且分组是array
                        ...
                        const childJsonSchemaProperties = jsonSchemaContent.items.properties;
                        let childJsonSchemaContent, childFieldValue, childFieldValidate = [], childClassNames = '', childWidgetType = '';
                        let childValidateItemPromises = [];
                        for (let child in childJsonSchemaProperties) {
                            if (uiSchema[fieldName].hasOwnProperty('items') && uiSchema[fieldName].items.hasOwnProperty(child)) {
                                childJsonSchemaContent = childJsonSchemaProperties[child];
                                childFieldValue = formData[fieldName] && Array.isArray(formData[fieldName]) && formData[fieldName].map(v => v[child]);
                                if (typeof uiSchema[fieldName].items[child] !== 'undefined') {
                                    childFieldValidate = (uiSchema[fieldName].items[child]['ui:options'] && uiSchema[fieldName].items[child]['ui:options'].validate) || [];
                                    childClassNames = uiSchema[fieldName].items.classNames || uiSchema[fieldName].items[child].classNames || '';
                                    childWidgetType = uiSchema[fieldName].items[child]['ui:widget'] || '';
                                }
                                childWidgetType = 'array';
                                //如果分组下 的控件 仍然一个array
                                let groupHasItem = false;
                                if (childJsonSchemaContent.type === 'array' && !childJsonSchemaContent.uniqueItems && childJsonSchemaContent.hasOwnProperty('items')) {
                                    if (typeof uiSchema[fieldName].items[child] !== 'undefined' && uiSchema[fieldName].items[child].hasOwnProperty('items')) {
                                        childFieldValidate = (uiSchema[fieldName].items[child].items['ui:options'] && uiSchema[fieldName].items[child].items['ui:options'].validate) || [];
                                        childClassNames = uiSchema[fieldName].items.classNames || uiSchema[fieldName].items[child].items.classNames || '';
                                        groupHasItem = true;
                                    }
                                }
                                childValidateItemPromises = this._validateField(childFieldValidate, child, childFieldValue, childClassNames, childWidgetType, fieldName);
                                let rPromise = _groupItemPromise(this, child, childValidateItemPromises, fieldName, groupHasItem);
                                validatePromises.push(rPromise);
                        ...
                        if (jsonSchemaContent.type === 'array' && !jsonSchemaContent.uniqueItems && uiSchema[fieldName].hasOwnProperty('items')) {
                            ...
                            let rPromise = _groupItemPromise(this, fieldName, validateItemPromises);
                            validatePromises.push(rPromise);
                            ...
                            validateItemPromises = this._validateField(fieldValidate, fieldName, fieldValue, classNames, widgetType);
                            let rPromise = _groupPromise(this, fieldName, validateItemPromises);
                            validatePromises.push(rPromise);
        ...
        function _groupItemPromise(_this, field, validateItemPromises, group = null, groupHasItem = false) {
            if (group) {
                //如果分组下控件仍然是array
                if (groupHasItem) {
                    return new Promise((resolve, reject) => {
                        Promise.all(validateItemPromises).then((values) => {
                            let resolveFieldName = values.length > 0 ? values[0].fieldName : field;
                            let resolveGroup = values.length > 0 ? values[0].groupField : group;
                            // 将该fieldName下的_errorType置为空
                            ...
                            let errorType = error.errorType;
                            let errorFieldName = error.fieldName;
                            let errorGroup = error.groupField;
                            let errorMessage, errorValidateCode;
                            // errorMessage, errorValidateCode字段只有动态校验器的场景才会出现，这种场景需要替换掉validate字段里面的message
                            if (typeof error.errorMessage !== 'undefined' &&
                                typeof error.errorValidateCode !== 'undefined') {
                                ...
                                errorMessage = error.errorMessage;
                                errorValidateCode = error.errorValidateCode;
                                uiSchema[errorGroup].items[errorFieldName].items['ui:options'].validate.map((validateItem, index) => {
                                    if (validateItem.validateCode === errorValidateCode) {
                    ...
                    return new Promise((resolve, reject) => {
                        Promise.all(validateItemPromises).then((values) => {
                            ...
                            if (typeof error.errorMessage !== 'undefined' &&
                                ...
                                errorMessage = error.errorMessage;
                                errorValidateCode = error.errorValidateCode;
                                uiSchema[errorGroup].items[errorFieldName]['ui:options'].validate.map((validateItem, index) => {
                                    if (validateItem.validateCode === errorValidateCode) {
                ...
                return new Promise((resolve, reject) => {
                    Promise.all(validateItemPromises).then((values) => {
                        ...
                        if (typeof error.errorMessage !== 'undefined' &&
                            ...
                            errorMessage = error.errorMessage;
                            errorValidateCode = error.errorValidateCode;
                            uiSchema[errorFieldName].items['ui:options'].validate.map((validateItem, index) => {
                                if (validateItem.validateCode === errorValidateCode) {
        ...
        function _groupPromise(_this, field, validateItemPromises, group = null, groupHasItem = false) {
            if (group) {
                if (groupHasItem) {
                    return new Promise((resolve, reject) => {
                        Promise.all(validateItemPromises).then((values) => {
                            ...
                            if (typeof error.errorMessage !== 'undefined' &&
                                ...
                                errorMessage = error.errorMessage;
                                errorValidateCode = error.errorValidateCode;
                                uiSchema[errorGroup][errorFieldName].items['ui:options'].validate.map((validateItem, index) => {
                                    if (validateItem.validateCode === errorValidateCode) {
                    ...
                    return new Promise((resolve, reject) => {
                        Promise.all(validateItemPromises).then((values) => {
                            ...
                            if (typeof error.errorMessage !== 'undefined' &&
                                ...
                                errorMessage = error.errorMessage;
                                errorValidateCode = error.errorValidateCode;
                                uiSchema[errorGroup][errorFieldName]['ui:options'].validate.map((validateItem, index) => {
                                    if (validateItem.validateCode === errorValidateCode) {
    ...
    _validateField(fieldValidate, fieldName, fieldValue, classNames, widgetType, groupField = null) {
            ...
            if (uiSchema.hasOwnProperty(field)) {
                ...
                if (uiSchema[field].hasOwnProperty('items')) {
                    ...
                    if (typeof uiSchema[field].items === 'undefined') {
                        ...
                        showOptions = uiSchema[field].items['ui:options'].show;
                        // 如果没有show字段证明该字段一直显示
                ...
                if (hasItems) {
                    if (typeof uiSchema[field].items === 'undefined') {
                        ...
                        // 防止属性重复添加
                        let classnames = uiSchema[field].items.classNames.split(' ');
                ...
                if (uiSchema[field]['ui:widget'] && (uiSchema[field]['ui:widget'] === 'group' || uiSchema[field]['ui:widget'] === 'Table')) {
                    let childShowOptions, childFieldHidden = false;
                    let groupUischema = uiSchema[field];
                    let codeArr = [];
                    sequence ? sequence.map(v => {
                        if (typeof v === 'object' && v.group === field) {
                            codeArr.push(v.code)
                        }
                    }) : ''
                    codeArr.map(code => {
                        if (groupUischema.hasOwnProperty(code)) {
                            if (typeof uiSchema[field][code] === 'undefined') {
                                ...
                                if (typeof childShowOptions === 'undefined') {
                                    ...
                                    //debugger;
                                    childFieldHidden = this._shouldFieldHidden(groupUischema, childShowOptions, formData[field]);
                                }
                            }
                            ...
                            if (typeof uiSchema[field][code] === 'undefined') {
                                ...
                                // 防止属性重复添加
                                let classnames = uiSchema[field][code].classNames.split(' ');
                                if (childFieldHidden && classnames.indexOf(XFORM_HIDDEN_CLASS) <= -1) {
                                    classnames.push(XFORM_HIDDEN_CLASS);
                                } else if (!childFieldHidden && classnames.indexOf(XFORM_HIDDEN_CLASS) > -1) {
                                    classnames.splice(classnames.indexOf(XFORM_HIDDEN_CLASS), 1);
                                }
                                uiSchema[field][code].classNames = classnames.join(' ');
                            } else {
                                uiSchema[field][code].classNames = (childFieldHidden ? XFORM_HIDDEN_CLASS : '');
                            }
                        }
    ...
    _filterExportData(uiSchema, formData) {
        // let filterFormData = { };
        let filterFormData = { uiSchema: uiSchema };
    ...
    _handleXFormChange(schema) {
        ...
        let currentFormData_ = deepCloneObject(currentFormData)
        for (let key in bizData) {
            if (bizData[key].type == 'Calculate' && (
                (uiSchema[key] && uiSchema[key]['ui:options'] && uiSchema[key]['ui:options'].calculateFields) ||
                false// group 或 Table 的计算 
            )) {
                let uiSchemaWidge = uiSchema[key]['ui:options'].calculateFields
                if (uiSchemaWidge.length) {
                    let calculateType = uiSchema[key]['ui:algorithm']
                    if (calculateType == 'average') {
                        //平均
                        currentFormData_[key] =
                            ((uiSchemaWidge.reduce((s, uw) => {
        ...
        let updateShowUISchema = this._updateFieldsShow(uiSchema, currentFormData_, this.state.sequence);

        // debugger
        // 执行表单校验
        this.validate(currentFormData_, changeFieldName);

        // 根据schema设置字段bizData中的displayName
        bizData = this._setAllFieldsDisplayName({ jsonSchema, uiSchema, formData: currentFormData_, bizData });
        ...
        this.setState({
            ...
            if (uiSchema[changeFieldName] && uiSchema[changeFieldName]['ui:options'] &&
                ...
                this._updateFieldsData(changeFieldName, currentFormData_[changeFieldName], uiSchema[changeFieldName]['ui:options'].updateFields);
            }
        });
    }
    ...
    _handleXFormSubmit(schema) {
        ...
        const uiSchema = this.state.uiSchema;
        // 获取联动后的uiSchema，保证通过_filterExportData输出的formData的正确
        let updateShowUISchema = this._updateFieldsShow(uiSchema, schema.formData, this.state.sequence);
        validateType = 'submit';
        //提交前校验
    ...
    XFormSubmit() {
        ...
        const uiSchema = this.state.uiSchema;
        // 获取联动后的uiSchema，保证通过_filterExportData输出的formData的正确
        let updateShowUISchema = this._updateFieldsShow(uiSchema, formData, this.state.sequence);
        // 提交前校验
    ...
    setButtonInputDisable(uiSchema, formData) {
        if (uiSchema) {
    ...
    _showPopView = (popWhat) => {
        let hasPopCode = false
        let popCode = ''
        let groupCode = []
        const uiSchema = this.state.uiSchema
        ...
        if (hasPopCode && popCode != '' && groupCode.length > 0 && groupCode.filter(x => x == popCode).length > 0) {
            this.setState({ popView: true, shouldPopCode: popWhat, })
        }
    }

    _closePopView = () => {
        this.setState({ popView: false, shouldPopCode: '', })
    }
    ...
    render() {
        ...
        let bizData = this.state.bizData;
        const popView = this.state.popView
        const shouldPopCode = this.state.shouldPopCode
        ...
        // console.log(seqJsonSchema, seqUiSchema, seqFormData);
        return (
