...
import SystemFieldOption from './systemFieldOption';
...
class FieldPicker extends PureComponent {
    ...
    constructor(...args) {
        ...
        this.handleSystemFieldsFoldChange = this.handleSystemFieldsFoldChange.bind(this);
        ...
        this.handleAddSystemField = this.handleAddSystemField.bind(this);
        this.handleAddCommonField = this.handleAddCommonField.bind(this);
        this.generateFieldDataFormSystemType = this.generateFieldDataFormSystemType.bind(this);
    ...
    // 业务字段、自定义字段区域折叠状态变化
    handleSystemFieldsFoldChange() {
        const {systemPanelFold} = this.state;
        this.setState({
    ...
    handleAddSystemField(event) {
        const code = event.currentTarget.getAttribute('data-code');
        const fieldData = this.generateFieldDataFormSystemType(code);
        if (fieldData !== null) {
            this.props.addFieldHandler(fieldData);
        }
    }
     ...
     // 添加“系统字段”
     generateFieldDataFormSystemType(code) {
    ...
    render() {
        const {messages, platform ,supportFieldList, registerWidgets, emptyPlaceholder,systemFields, commonFields} = this.props;
        const {keyword, searchKeyword, commonPanelFold, customPanelFold,systemPanelFold} = this.state;
        let configFields = [];
        ...
        // 根据关键词进行列表选项过滤
        const filterSystemFields = this._filterListItemByLabel(systemFields, searchKeyword);
            ...
            <div className="app-xform-builder-field-picker-wrapper">
                ...
                {(() => {
                    if (filterSystemFields.length <= 0 && filterCommonFields.length <= 0 && filterCustomFields.length <= 0) {
                        ...
                        return (
                            <div>
                                 {filterSystemFields.length > 0 && <div className="common-field-template">
                                    <div className={classnames({ 'field-panel': true, fold: systemPanelFold })} onClick={this.handleSystemFieldsFoldChange} >
                                        <i className="xform-iconfont icon">&#xe8ec;</i>
                                        <span className="template-title">
                                            {messages[getMessageId('fieldPickerSystemTitle')]}
                                        </span>
                                    </div>
                                    {/* 系统控件 */}
                                    {
                                        !systemPanelFold && <ul className="template-list">
                                            {filterSystemFields.map((field, index) => {
                                                return (
                                                    <li key={field.code} className="common-type">
                                                        <SystemFieldOption
                                                            index={index}
                                                            field={field}
                                                            addSystemFieldHandler={this.handleAddSystemField}
                                                            generateFieldData={this.generateFieldDataFormSystemType}
                                                        />
                                                    </li>
                                                );
                                            })}
                                ...
                                {filterCommonFields.length > 0 && <div className="common-field-template">
                                    <div className={classnames({ 'field-panel': true, fold: commonPanelFold })} onClick={this.handleCommonFieldsFoldChange} >
                                        <i className="xform-iconfont icon">&#xe8ec;</i>
                                        <span className="template-title">
