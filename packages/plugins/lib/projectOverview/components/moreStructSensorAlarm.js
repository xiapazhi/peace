...
const moreStructSensorAlarm = (props) => {
    const { dispatch, structureSelected } = props
    ...
    let [factors, setFactors] = useSafeState([]);
    let [sensorNum, setSensorNum] = useSafeState(0);
    let [warningData, setWarningData] = useSafeState(initWarningData);
    let [lastestWarningLevel, setLastestWarningLevel] = useSafeState('正常');
    let [lastestWarningColor, setLastestWarningColor] = useSafeState('#2F53EA');
    ...
    useEffect(() => {
        if (structureSelected) {
            dispatch(getFactorsByStructures(structureSelected)).then(res => {
                if (res.success) {
                    let factorList = [];
                    //let sensorNum = 0;
            ...
            dispatch(getAlarmsByStructure(structureSelected, 'new')).then(res => {
                if (res.success) {
                    ...
                    factors.map((sd, index) => {
                        return (
                            <Col span={4} className='sensorDataCol' key={index}>
                                <Input addonAfter={<div className="sensorDataColAddonAfter">{sd.items ? sd.items.length : 0}</div>}
...
export default connect(mapStateToProps)(moreStructSensorAlarm);
