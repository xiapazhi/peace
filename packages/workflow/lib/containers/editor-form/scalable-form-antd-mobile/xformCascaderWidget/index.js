...
})("f879b47e6f41e6adc55ec7447ae18d27", ".xform-custom-cascader-select.disabled .am-list-content,.xform-custom-cascader-select.disabled .am-list-extra{color:#bbb}");
...
var CustomCascader = /*#__PURE__*/function (_PureComponent) {
  ...
  _createClass(CustomCascader, [{
    ...
    key: "_getTreeDepth",
    value: function _getTreeDepth(tree) {
      var _this = this;

      var depths = [];

      if (tree && tree.children && tree.children.length > 0) {
        depths = tree.children.map(function (treeNode) {
          return _this._getTreeDepth(treeNode);
        });
        return Math.max.apply(Math, _toConsumableArray(depths)) + 1;
      } else {
        return 1;
      }
    }
    ...
    value: function render() {
      ...
      var treeDepths = enumOptions.map(function (enumItem) {
        return _this2._getTreeDepth(enumItem);
      });
      var colNumber = Math.max.apply(Math, _toConsumableArray(treeDepths)); //解析schema中约定的_errorType字段，用来标识是validate中哪种类型校验不通过
      ...
      }, /*#__PURE__*/_react["default"].createElement(_antdMobile.Picker, _extends({
        data: enumOptions,
        cols: colNumber,
        value: value,
        disabled: disabled,
