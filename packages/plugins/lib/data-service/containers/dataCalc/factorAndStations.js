...
import {
  Form, Select, Cascader, Input, Button, Row, Col, message, Alert,
} from 'antd';
...
class FactorAndStations extends Component {
      ...
      this.props.factors.map((s) => {
        if (s.checked) {
          newData.push({
            value: s.id,
            label: s.name,
            children: s.items.map((k) => ({ value: k.id, label: k.name })),
          });
        }
      });
    }
  ...
  getJson = (values) => {
    ...
    switch (this.props.methodType) {
      case 'interrupt':
        paramJson = { thr_int: Number(values.iv) };
        break;
      case 'burr':
        paramJson = { thr_burr: Number(values.bv1) };
        break;
      case 'trend':
        paramJson = {
  ...
  getStationstoSave = () => {
    const toSave = []; const toSaveName = []; const notToSave = []; const
      notToSaveName = [];
    const selectIds = this.props.stationsValue;
    for (let i = 0; i < selectIds.length; i++) {
      const seStaName = this.props.stations.find((a) => a.id == selectIds[i]).name;
      let cfg;
      if (this.props.methodType == 'interrupt') {
        ...
        const type = this.props.methodType == 'burr' ? 2 : 3;
  ...
  handleSave = () => {
    ...
    form.validateFields().then((values) => {
      if (this.props.stationsValue.length != 0) {
        const ids = this.getStationstoSave();
        if (ids.toSave.length != 0) {
          const paramJson = this.getJson(values);
          const data = {
          ...
          const pushData = data.station.map((d) => ({
            station: d, // 测点
            factorId: this.props.factorId,
          ...
          if (ids.notToSave.length != 0) {
            const fact = this.props.methodType == 'interrupt' ? '因素' : '子项';
            this.setState({
          ...
          const sg = this.props.methodType == 'interrupt' ? '因素下的中断'
  ...
  render() {
    const {
      factorId, structures, factors, stations, abnMethods,
    } = this.props;
      ...
      <div style={{ marginTop: 15 }}>
        ...
        >
          <Form.Item>
            <Cascader
              id="factor"
              value={[this.props.factorId, this.props.itemId]}
              onChange={this.onFactorChange}
              style={{ textAlign: 'left', width: 274 }}
              options={this.renderFactorItems()}
              placeholder="请选择监测项"
            />
          </Form.Item>
          <Form.Item>
...
function mapStateToProps(state) {
  const {
    auth, thresholdStructures, abnMethods, abnParamList, thresholdFactorList, thresholdStationList,
  } = state;
  const isRequesting = thresholdStructures.isRequesting || thresholdFactorList.isRequesting;
  ...
  return {
    ...
    factors: thresholdFactorList.data,
