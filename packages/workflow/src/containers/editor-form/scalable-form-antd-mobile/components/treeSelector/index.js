...
class TreeSelector extends PureComponent {
  ...
  getSelectedValueListFromTree(tree = {}, value) {
    ...
    if (item) {
      ...
      if (item.parent !== utils.getTreeRootValue()) {
  ...
  render() {
    ...
    const selectedItem = this.getItemFromTree(this.props.tree, this.state.selectedValueList[this.state.selectedValueList.length - 1]) || { label: '', title: '' };
    return (
