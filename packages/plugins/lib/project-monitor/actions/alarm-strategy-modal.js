...
export function getProjectList(userId) {
  return dispatch => basicAction({
      ...
      reducer: {
          name: 'strategyProjectList'
      }
  });
}

export const STRUCTS_LIST_REQUEST = 'STRUCTS_LIST_REQUEST';
export const STRUCTS_LIST_SUCCESS = 'STRUCTS_LIST_SUCCESS';
export const STRUCTS_LIST_FAILURE = "STRUCTS_LIST_FAILURE";

export function getStrategyStructsList(orgId, idArr = []) {
  
  return dispatch => basicAction({
      type: 'get',
      dispatch: dispatch,
      actionType: 'STRATEGY_STRUCTS_LIST',
      url: ApiTable.getStructsList.replace("{organizationId}", orgId) + `?projects=${idArr.join(",")}`,
      msg: {
          error: '获取结构物信息失败'
      },
      reducer: {
          name: 'strategyStructsList'
      }
  });
