        ...
        <Row>
          ...
          <Col span={22}>
            ...
            <p style={{ marginTop: 10 }}>
              实时告警：
              {alarmMsg.new.length > 0 ? '' : '无'}
              {' '}
            </p>
            {
              alarmMsg.new.length > 0 ? alarmMsg.new.map((s) => (
                <Row style={{ marginTop: 7 }}>
                  <Col span="1" />
                  <Col span="23" style={{}}>{s.alarmContent}</Col>
                  <Col span="1" />
                  <Col span="23" style={alarmMsgColStyle}>
                    产生次数：
                    {s.alarmCount}
                    次
                  </Col>
            ...
            <p style={{ marginTop: 21 }}>
              历史告警：
              {alarmMsg.history.length > 0 ? '' : '无'}
              {' '}
            </p>
                ...
                alarmMsg.history.length > 0 ? alarmMsg.history.map((s) => (
                  <Timeline.Item>
                    <p>
                      ...
                      /
                      {' '}
                      <span style={alarmMsgColStyle}>
                        {' '}
                        {s.startTime}
                      </span>
                    </p>
                    <p style={alarmMsgColStyle}>
                      产生次数：
                      {s.alarmCount}
                      次
                    </p>
