...
export default class CustomObjectFieldTemplate extends Component {
    ...
    render() {
        const { TitleField, properties, title, description, uiSchema, idSchema,
            formContext: { popView, closePopView, shouldPopCode, currentNode, isFieldList, schema } } = this.props;
        ...
        const curWidgetId = idSchema['$id'].split('_')[1]
        const isTableGroup = uiSchema && ((uiSchema['ui:widget'] && uiSchema['ui:widget'] == 'Table')
            || (uiSchema[curWidgetId] && uiSchema[curWidgetId]['items'] && uiSchema[curWidgetId]['items']['ui:widget'] == 'Table'));

        const isTableArray = isTableGroup && schema.properties[curWidgetId] && schema.properties[curWidgetId].type == 'array';
        ...
        return (
            <div className={classnames({
                'object_row_group_wraper': options.groupName && !isTableArray ? true : false,
                'object_wraper': true,
                'object_margin': isGroup && !uiSchema.hasOwnProperty('items')
            })} style={isTableGroup && !isTableArray ? {
                overflow: 'auto'
            } : {}, isTableGroup ? {} : {}}>
                ...
                <div className={classnames({ 'object_row_group_row': options.groupName ? true : false, 'object_row': true })}
                    style={isTableGroup ? { whiteSpace: 'nowrap', display: 'block' } : {}}>
                    {
                        properties.map(prop => {
                            ...
                            let style = Number(itemNumberInRow) > 0 ? { width: (itemNumberInRow + '%'), marginTop: 10, marginLeft: (itemMarginLeft + '%'), paddingRight: 10 } : { display: 'none' };
                            ...
                            return (
                                <div style={Object.assign({}, style, isTableGroup ? {
                                    display: 'inline-block', width: 'auto', position: 'relative'
                                } : {})} className={addClassNames} key={prop.content.key}>
                                    {prop.content}
                                </div>
                            )
                        })
