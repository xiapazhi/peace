...
const actionCreators = {
    ...
    fetchSelectedFieldData(params, filterSystemFields, messages) {
        const {systemFieldSupport, langConfigSupport, formCode, lang, customInterfaces, customGateway, namespace, env, bpmnNodes} = params;
                ...
                request.fetch(request.getInterface(apiName, customInterfaces, env), requestParam, customGateway, env).then((originData) => {
                    ...
                    if (useGetSchemaByCode && originData) {
                        if (isSchemaLegal(originData)) {
                            data = getFieldsBySchema(originData,bpmnNodes);
                        } else {
        ...
        const getFieldDataSourceList = () => new Promise((resolve, reject) => {
           if(env === 'dev'){
            ...
            //    }, customGateway, env, {type: 'POST'}).then(resolve).catch(reject);
               fetch('http://rest.apizza.net/mock/ed27f575082bc8b08597a0476ea1a8f5/memberList').then(res => res.json()).then(resolve).catch(reject);
           }
        });
        return (dispatch) => {
        ...
        return (dispatch) => {
            const getDataSourceServerUrl = () => new Promise((resolve, reject) => {
                if(env === 'dev'){
                    ...
                    //     })
                    // }, customGateway, env, {type: 'POST'}).then(resolve).catch(reject);
                    fetch('http://rest.apizza.net/mock/ed27f575082bc8b08597a0476ea1a8f5/datasourseList').then(res=>res.json()).then(resolve).catch(reject);
                }
            });
