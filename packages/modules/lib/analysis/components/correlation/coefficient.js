...
import { Card, Select, Row, Col, Slider } from 'antd';
import moment from 'moment';
import { Chart } from '@peace/components';

const { CommonPointChart, ColumnChart } = Chart;
...
class Coefficient extends Component {
    ...
    renderPointChart = (dataSource) => {
        ...
        if (dataSource.dependentValuesMax == dataSource.dependentValuesMin ||
            ...
            return <span><FrownOutlined /> 相关联数据，不满足相关系数计算公式，分母不能为0。</span>
        }
        ...
        return data.length > 0 ? <div>
            <Row gutter={2} style={{ marginTop: 35 }}>
                <Col md={24} sm={24}>
                    <CommonPointChart data={data} height={300} scaleConfig={scaleConfig} />
                </Col>
            </Row>
            ...
            : <span><FrownOutlined /> 暂无关联数据</span>
    }
    //取slider范围
    ...
    renderIntervalChart = (dataSource) => {
        ...
        if (conditionData.length > 0) {
            ...
            return <div>
                ...
                <Row gutter={2} style={{ marginTop: 2 }}>
                    <Col md={24} sm={24}>
                        <ColumnChart data={data} height={300}
                            scaleConfig={scaleConfig} configs={{
                                xAxis: 'value',
                                yAxis: 'freq',
    ...
    render() {
        ...
        let indArray = independentVariableStaionId ? independentVariableStaionId.split('-') : [];
        let indString = "";
        for (let i = 0; i < indArray.length - 1; i++) {
            let a = i == 0 ? indArray[i] : '-' + indArray[i];
            indString += a;
        }

        let dArray = dependentVariableStaionId ? dependentVariableStaionId.split('-') : [];
        let dString = "";
        for (let i = 0; i < dArray.length - 1; i++) {
            let a = i == 0 ? dArray[i] : '-' + dArray[i];
            dString += a;
        }
        let ind = independentVariableStaionId ? indString : null;
        let d = dependentVariableStaionId ? dString : null;
        return (
            <Card style={{ marginTop: 15 }} title={<div className="A14">
                <div style={{ display: 'inline-block', marginRight: 40 }}>
                    <label style={{ marginRight: 10 }}>关联自变量</label>
                    <Select placeholder="请选择自变量测点" style={{ width: 150 }}
                        value={ind} onChange={this.onIndependentVariableStaionChange} >
                        {
                ...
                <div style={{ display: 'inline-block' }}>
                    ...
                    <Select placeholder="请选择因变量测点" style={{ width: 150 }}
                        value={d} onChange={this.onDependentVariableStaionChange} >
                        {
