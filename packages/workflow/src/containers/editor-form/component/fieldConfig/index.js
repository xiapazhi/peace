...
import { Button, Row, Col, Modal, Checkbox, Popconfirm, message } from 'antd';
...
import GroupOperateDB from '../fieldOptionList/group'
...
class FieldConfig extends Component {
    ...
    toggleBlockFoldChange(block) {
        const { basicBlockFold, advancedBlockFold, optionBlockFold, cascadeBlockFold, relevanceBlockFold, dboperateBlockFold, } = this.state;
        switch (block) {
            case 'basic':
                this.setState({
    ...
    schema2ConfigFormDataConverter(editFieldData, formData) {
        ...
        //console.log(jsonSchemaContent,uiSchemaContent);
            ...
            switch (key) {
                ...
                case 'showGroupTitle':
                    if (editFieldData[key]) {
                        ...
                        resultData[key] = editFieldData.uiSchema
                            && editFieldData.uiSchema[editFieldData.code]
                            && editFieldData.uiSchema[editFieldData.code]['ui:options']
                            && editFieldData.uiSchema[editFieldData.code]['ui:options'][key] ?
                            editFieldData.uiSchema[editFieldData.code]['ui:options'][key] : ''
                    }
                    break;
                case 'algorithm':
    ...
    configFormData2schemaConvertor(editFieldData, formData) {
        ...
        const bpmnNodes = this.props.bpmnNodes;
        ...
        Object.keys(formData).map((key) => {
            ...
            switch (key) {
                ...
                case 'algorithm':
                    ...
                    break;
                default:
                    console.log(`configSchema中获取到不能识别的字段[${key}]设置，请注意是否传入了相应的自定义字段处理方法`);
            }
            // 插入自定义配置项处理
            registerWidgets.map((widget) => {
                if (widget.customConfigFields && widget.customConfigFields.length > 0) {
    ...
    generateFieldsWithDataSource(type) {
    ...
    updateDataSourceConfigConfirm() {
        ...
        let key = this.state.updateConfigType == 'relevance' ? 'calculateFields' : 'updateFields'
        if (typeof updateFields !== 'undefined' && updateFields.length > 0) {
            ...
            if (typeof uiSchemaContent['ui:options'] !== 'undefined' &&
                typeof uiSchemaContent['ui:options'][key] !== 'undefined') {
    ...
    render() {
        const { globalConfig, customInterfaces, customGateway, customUploadRequest, onImagePreview, xformBizData, xformOptionBizData, editFieldData,
            dataSource, serverCode, updateFieldDataHandler, messages, locale, supportFieldList, supportConfigList, registerWidgets, emptyPlaceholder,
            popupContainer, bpmnNodes, dispatch, fields, operateDBGroup, user, commonFields, } = this.props;
        ...
        if (editFieldData !== null) {
            if (configSchema.hasOwnProperty(editFieldData.type)) {
                ...
                if (configSchemaJsonSchema.properties.shownodes && Array.isArray(bpmnNodes)) {
                    ...
                    configSchemaJsonSchema.properties.shownodes.items.enum = bpmnNodes.map(i => i.id);
                    configSchemaJsonSchema.properties.shownodes.items.enumNames = bpmnNodes.map(i => i.name);
                }
                if (configSchemaJsonSchema.properties.disnodes && Array.isArray(bpmnNodes)) {

                    configSchemaJsonSchema.properties.disnodes.items.enum = bpmnNodes.map(i => i.id);
                    configSchemaJsonSchema.properties.disnodes.items.enumNames = bpmnNodes.map(i => i.name);
                }
                ...
                if (hasDBOperateButton) {
                    groupDBOperateView = (
                        <GroupOperateDB
                            ...
                            user={user}
                            updateFields={(code) => this.props.updateFieldsApp(code)}
        ...
        const { basicBlockFold, advancedBlockFold, optionBlockFold, cascadeBlockFold, relevanceBlockFold, dboperateBlockFold, showConfigModalVisible, fieldShowConfig, currentEditShowOptionCode,
            updateConfigModalVisible, updateConfigType, dataSourceFieldList } = this.state;
        const beneathFields = fieldShowConfig.beneathFields;
        const showConfig = fieldShowConfig.showConfig;
        const updateFieldConfigModal4Relevance = updateConfigType == 'relevance'
        return (
            <div className="app-xform-builder-field-config-wrapper">
                    ...
                    if (editFieldData !== null) {
                        return (
                            <div className="field-config-block">
                                {/* 基础属性 */}
                                <div className={classnames({ 'field-config-block-item': true, fold: basicBlockFold })}>
                                    <div className="field-config-block-header" onClick={() => { this.toggleBlockFoldChange('basic'); }}>
                                        <i className="xform-iconfont expand-icon">&#xe8ec;</i>
                                        <span>{messages[getMessageId('fieldConfigBoxBasicTitle')]}</span>
                                    </div>
                                    {!basicBlockFold && <div className="field-config-block-content"> {basicConfigXForm} </div>}
                                </div>
                                {/* 选项 */}
                                {listItemConfig !== '' && <div className={classnames({ 'field-config-block-item': true, fold: optionBlockFold })}>
                                    <div className="field-config-block-header" onClick={() => { this.toggleBlockFoldChange('option'); }}>
                                        <i className="xform-iconfont expand-icon">&#xe8ec;</i>
                                        <span>{messages[getMessageId('fieldConfigBoxOptionTitle')]}</span>
                                    </div>
                                    {!optionBlockFold && <div className="field-config-block-content"> {listItemConfig} </div>}
                                </div>}
                                {/* 数据源 */}
                                <div className={classnames({ 'field-config-block-item': true, fold: advancedBlockFold })}>
                                    <div className="field-config-block-header" onClick={() => { this.toggleBlockFoldChange('advance'); }}>
                                        <i className="xform-iconfont expand-icon">&#xe8ec;</i>
                                        <span>{messages[getMessageId('fieldConfigBoxAdvanceTitle')]}</span>
                                    </div>
                                    {
                                        !advancedBlockFold &&
                                        <div className="field-config-block-content">
                                            {advanceConfigXForm}
                                            {bizConfigXForm}
                                        </div>
                                    }
                                ...
                                {/* 级联 */}
                                {editFieldData.fieldType !== 'system' && <div className={classnames({ 'field-config-block-item': true, fold: cascadeBlockFold })}>
                                    <div className="field-config-block-header" onClick={() => { this.toggleBlockFoldChange('cascade'); }}>
                                        <i className="xform-iconfont expand-icon">&#xe8ec;</i>
                                        <span>{messages[getMessageId('fieldConfigBoxCascadeTitle')]}</span>
                                    </div>
                                    {!cascadeBlockFold && <div className="field-config-block-content"> {cascadeConfig} </div>}
                                </div>}
                                {/* 关联 */}
                                {editFieldData.type == "Calculate" && <div className={classnames({ 'field-config-block-item': true, fold: relevanceBlockFold })}>
                                    <div className="field-config-block-header" onClick={() => { this.toggleBlockFoldChange('relevance'); }}>
                                        <i className="xform-iconfont expand-icon">&#xe8ec;</i>
                                        <span>{messages[getMessageId('fieldConfigBoxRelevanceTitle')]}</span>
                                    </div>
                                    {!relevanceBlockFold && <div className="field-config-block-content">
                                        <div className="cascade-config">
                                            <div className="cascade-config-panel">
                                                <Button
                                                    type="primary"
                                                    onClick={() => this.handleFieldUpdateModalOpen('relevance')}
                                                >
                                                    {messages[getMessageId('fieldConfigControlRelatedTitle')]}
                                                </Button>
                                                {/* <Popconfirm
                                                    title={messages[getMessageId('fieldConfigClearRelatedPopconfirmTitle')]}
                                                    onConfirm={this.handleClearAll}
                                                    getPopupContainer={popupContainer}
                                                >
                                                    <a href="#">
                                                        {messages[getMessageId('fieldConfigClearRelatedConfigTitle')]}
                                                    </a>
                                                </Popconfirm> */}
                                            </div>
                                        </div>
                                ...
                                {/* 保存业务控件 */}
                                {
                                    hasDBOperateButton && <div className={classnames({ 'field-config-block-item': true, fold: dboperateBlockFold, })}>
                                        <div className="field-config-block-header" onClick={() => { this.toggleBlockFoldChange('dboperate'); }}>
                                            <i className="xform-iconfont expand-icon">&#xe8ec;</i>
                                            <span>业务控件</span>
                                        </div>
                                        {!dboperateBlockFold && <div className="field-config-block-content"> {groupDBOperateView} </div>}
                                    </div>
                                }
                            </div>
                        );
                                            ...
                                            {showConfig.map((showConfigItem, index) => {
                                                ...
                                                return (
                                                    <li
                                                        ...
                                                        key={`option-item-${showConfigItem.code}-${index}`}
                                                    ...
                                                    {beneathFields.map((field, index) => {
                                                        ...
                                                        return (
                                                            <li
                                                                ...
                                                                key={`field-item-${field.code}-${index}`}
                ...
                <If condition={updateConfigModalVisible}>
                    <Then>
                        <Modal
                            title={messages[getMessageId(
                                updateFieldConfigModal4Relevance ?
                            ...
                            footer={[
                                ...
                                updateFieldConfigModal4Relevance ?
                                    <Popconfirm
                                        title={messages[getMessageId(updateFieldConfigModal4Relevance ? 'updateFieldConfigRelevanceModalClearConfirmTitle' : 'updateFieldConfigModalClearConfirmTitle')]}
                                        onConfirm={() => {
                                            this.handleClearFieldUpdate();
                                            message.success(messages[getMessageId('updateFieldConfigModalClearSuccessTip')]);
                                            this.setState({
                                                updateConfigModalVisible: false,
                                                updateConfigType: null,
                                            });
                                    ...
                                    >
                                        <Button
                                            type="ghost"
                                        >{messages[getMessageId(updateFieldConfigModal4Relevance ? 'updateFieldConfigRelevanceModalClear' : 'updateFieldConfigModalClear')]}</Button>
                                    </Popconfirm>
                                    : ''
                            ]}
                            ...
                            <div className="update-dataSource-list">
                                <If condition={dataSourceFieldList.length > 0}>
                                            ...
                                            {dataSourceFieldList.map((field, index) => {
                                                ...
                                                return (
                                                    <li className="update-dataSource-item"
                                                        key={`update-dataSource-item-${field.code}-${index}`}
                                                        data-fieldcode={field.code}
                                    ...
                                    <Else>{() => {
                                        return (
                                            <p className="empty">
                                                {messages[getMessageId(updateFieldConfigModal4Relevance ? 'updateFieldConfigRelevanceModalEmptyOption' : 'updateFieldConfigModalEmptyOption')]}
                                            </p>
                                        );
                                    }}</Else>
                                </If>
