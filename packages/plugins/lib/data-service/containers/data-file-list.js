...
import { getDataFileDir, downloadDataFile } from '../actions/dataFiles';
...
class DataFileList extends Component {
    ...
    render() {
        ...
        const dataSource = this.state.path.length == 0 ?
            [{ id: 1, length: 0, path: '测点数据', type: 'DIRECTORY' }, { id: 2, length: 0, path: '设备数据', type: 'DIRECTORY' }]
            : this.props.dir;
        return (
            <div>
                <Row style={{ padding: '24px 0 0 24px' }}>
                    <Col span="12">
                        <Breadcrumb>
                            ...
                            {
                                this.state.path.map((p,index) => <Breadcrumb.Item key={index}><a onClick={() => this.changePath(p)}>{p}</a></Breadcrumb.Item>)
                            }
                        </Breadcrumb>
                    </Col>
                    <Col span="12">
                ...
                <Table rowKey='id' loading={this.props.isRequesting} dataSource={dataSource} columns={columns} style={{ padding: 24 }} />
