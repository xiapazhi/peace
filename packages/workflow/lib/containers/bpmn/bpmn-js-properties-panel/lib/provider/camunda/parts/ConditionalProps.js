...
module.exports = function (group, element, bpmnFactory, translate) {
  ...
  group.entries.push({
    ...
    html: '<div class="bpp-row bpp-hide">' + '<label for="cam-condition-type">' + escapeHTML(translate('Condition Type')) + '</label>' + '<div class="bpp-field-wrapper">' + '<select id="cam-condition-type" name="conditionType" data-value>' + //'<option value="pass">'+ escapeHTML(translate('同意')) + '</option>' +
    //'<option value="reject">'+ escapeHTML(translate('不同意')) + '</option>' +
    '<option value="expression">' + escapeHTML(translate('Expression')) + '</option>' + '<option value="script">' + escapeHTML(translate('Script')) + '</option>' + '<option value="" selected></option>' + '</select>' + '</div>' + '</div>' + // expression
    '<div class="bpp-row bpp-hide">' + '<label for="cam-condition" data-show="isExpression">' + escapeHTML(translate('Expression')) + '</label>' + '<div class="bpp-field-wrapper" data-show="isExpression">' + '<input id="cam-condition" type="text" name="condition" />' + '<button class="clear" data-action="clear" data-show="canClear">' + '<span>X</span>' + '</button>' + // '<input type="button" class="btn-select"  value="设置流转条件" onclick="openSetExpressions(1)"/>' +
    '</div>' + '<div data-show="isScript" class="bpp-hide">' + script.template + '</div>' + '</div>' + '<div class="bpp-row">' + '<label for="cam-multiInstance-type">' + escapeHTML('流转条件配置') + '</label>' + '<div class="bpp-field-wrapper">' + '<input type="button" class="btn-select"  value="编辑" onclick="openBpmnModal(\'conditionExpression\')"/>' + '</div>' + '</div>',
    ...
    isExpression: function (element, inputNode) {
      ...
      if (conditionType.selectedIndex >= 0) {
        // console.log(conditionType.options[conditionType.selectedIndex].value)
