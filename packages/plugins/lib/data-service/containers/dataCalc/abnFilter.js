...
import {
  Button, Card, Table, Tag, Popconfirm, Switch, Row, Col, message, Input, Divider,
} from 'antd';
import { SearchOutlined } from '@ant-design/icons';
import { PinyinHelper } from '@peace/utils';
import {
  getAbnFilterMethods, getAbnFilterCfgs, changeCfgStatus, deleteAbnFilterCfg, batchAbnFilterCfgs,
...
class AbnFilter extends Component {
  ...
  render() {
    ...
    // 按监测因素
    const cfgs = filterCfgs && filterCfgs.filter((c) => c.factorId == this.props.cfgFilterFactorId
                && (searchValue
                  ? PinyinHelper.isPinyinMatched(c.stationName, searchValue)
                  : true));
    const batchDisplay = { display: cfgs && cfgs.length > 0 ? 'inline-block' : 'none', float: 'right' };
    const columns = [
      {
        title: '测点',
