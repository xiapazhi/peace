...
class Recalc extends Component {
  ...
  componentDidMount() {
    const {
      dispatch, params, structId, factorId,
    } = this.props;
    this.init(dispatch, structId, factorId);
  }
    ...
    const columns = [
      {
        ...
        key: 'deviceName',
        render: (text, record) => <b>{record.deviceName}</b>,
      }, {
        title: '计算公式',
        dataIndex: 'dformula',
        key: 'dformula',
        render: (text, record) => <b>{record.dformula}</b>,
      }, {
        title: '计算参数',
        dataIndex: 'dparams',
        key: 'dparams',
        render: (text, record) => <b>{record.dparams}</b>,
      }, {
        title: '参数值',
        dataIndex: 'dpvalue',
        key: 'dpvalue',
        render: (text, record) => <b>{record.dpvalue}</b>,
      }];
  ...
  onBatchRecal = () => {
    const {
      structId, factorId, startTime, endTime, user,
    } = this.props;
    if (this.state.selectedRowKeys.length <= 0) {
      ...
      const tmpCatch = this.state.recalcIntervals;
      stationIds.forEach((s) => {
        if (this.state.recalcIntervals.get(s) != undefined) {
          clearInterval(this.state.recalcIntervals.get(s));
        }
        this.props.dispatch(issueRecalcRequest({
          name: 'recalc_task', userId: user.id, stationId: s, startTime, endTime,
        })).then((data) => {
          const task = data.payload.data.msgId;
          const intervalHandle = this.timingWorker(s, task);
  ...
  timingWorker = (d, task) => {
    ...
    return sivResult = setInterval(() => {
      ...
      this.props.dispatch(getIssueRecalcResult(task)).then((data) => {
        if (data.success) {
          const result = data.payload.data.state;
          resultArray.push(result);
          const res = `同步${g_RecalcResultMap[`${result}`].desp}`;
          let choose = [];
          ...
          if (timeOut == 0) {
            ...
            const res = result == g_RecalcResult.WAITING
              ? '重计算超时, 请重新下发' : `同步${g_RecalcResultMap[`${result}`].desp}`;
            this.catchResult(d, [res]);
          }
        } else {
          let choose = [];
          const res = `同步配置${g_RecalcResultMap['2'].desp},请重新下发`;
          choose = [res];
          this.catchResult(d, [res]);
        }
      });
  ...
  handleInputChange = (e) => {
    if (e.target.value != null) {
      ...
      const func = ((ep) => ((s) => (s.stationLoc).search(ep) > -1))(value);
  ...
  render() {
    ...
    const {
      structId, factorId, startTime, endTime,
    } = this.props;
    let tmpds = [];
...
function mapStateToProps(state) {
  const {
    recalList, auth, issueTask, resultTask,
  } = state;
  return {
